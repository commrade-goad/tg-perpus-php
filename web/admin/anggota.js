...
function editUser(userId, name, tipe) {
    ...
    const modal_name = document.getElementById('name-new');
...
$("#edit-user-form").submit(function(event) {
    ...
    $.ajax({
        ...
        url: `/api/edit_user/index.php`, // Ganti dengan endpoint yang benar untuk memperbarui data pengguna
        data: formData,
        success: function(response) {
            if (response.success) {
...
$(document).ready(function() {
    ...
    function filterUsers(query) {
        ...
        rows.each(function() {
            ...
            const name = row.find('td').eq(1).text().toLowerCase(); // Ambil kolom Tipe
            const type = row.find('td').eq(2).text().toLowerCase(); // Ambil kolom Tipe

            // Cek apakah NPM atau Type cocok dengan query
            if (npm.includes(query) || type.includes(query) || name.includes(query)) {
...
<td class="px-4 py-2">
<button class="bg-blue-400 hover:bg-blue-700 px-2 py-1 rounded mr-2" onclick="editUser(${user.id}, '${user.name}', ${user.type})" data-user-id="${user.id}">Edit</button>
